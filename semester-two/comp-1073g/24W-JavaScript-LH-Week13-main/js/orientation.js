...
function error() {
    statusMsg.textContent = 'Device orientation not supported by your browser!';
}
...
if (!window.DeviceOrientationEvent){
    ...
    betaValue.textContent = Math.round(event.alpha) + "°";
    ...
    gammaValue.textContent = Math.round(event.alpha) + "°";
    betaSlider.value = Math.round(event.alpha);
    gammaSlider.value = Math.round(event.alpha);
